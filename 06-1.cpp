#include <sstream>
#include <functional>
#include <numeric>

#include "common.hpp"

int main(int argc, char *argv[])
{
	std::ifstream file = aoc::open_data(argc, argv);
	std::string record;

	std::vector<std::vector<ull>> numbers;
	std::vector<std::pair<ull, std::function<ull(ull, ull)>>> ops;

	auto nproblems = 0U;
	while (std::getline(file, record)) {
		//std::cout << record << std::endl;
		std::string field;

		if (0 == nproblems) {
			std::stringstream probe_rec(record);
			while (probe_rec >> field)
				nproblems++;

			numbers.resize(nproblems, std::vector<ull>());
			ops.reserve(nproblems);
		}

		std::stringstream rec(record);
		auto i = 0U;
		while (rec >> field) {
			auto sym = field[0];
			switch (sym) {
			case '*':
				ops.emplace_back(1ULL, std::multiplies<ull>());
				break;
			case '+':
				ops.emplace_back(0ULL, std::plus<ull>());
				break;
			default:
				numbers[i].emplace_back(std::stoll(field));
			}
			i++;
		}
	}

	auto result = 0ULL;
	std::vector<ull> partials;
	for (auto i = 0U; i < nproblems; i++) {
		result += std::accumulate(numbers[i].begin(), numbers[i].end(),
			ops[i].first, ops[i].second);
	}

	std::cout << "Result: " << result << std::endl;

	return 0;
}
